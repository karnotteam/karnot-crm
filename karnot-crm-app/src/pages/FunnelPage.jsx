...
// ----------------------------------------------------------------------
// 1. SMART OPPORTUNITY MODAL (Unchanged - works well)
// ----------------------------------------------------------------------
...
// ----------------------------------------------------------------------
// 2. INTERACTION MODAL (Modified to read interactions from COMPANY)
// ----------------------------------------------------------------------
const OpportunityDetailModal = ({ opp, onClose, onSaveInteraction, quotes = [], contacts = [], companies = [] }) => {
    const [activeTab, setActiveTab] = useState('ACTIVITY');
    const [logType, setLogType] = useState('Call');
    const [logOutcome, setLogOutcome] = useState('');
    const [logDate, setLogDate] = useState(new Date().toISOString().split('T')[0]);

    // FIND THE MASTER COMPANY OBJECT
    ...
    const companyContacts = (contacts || []).filter(c => c.companyId === opp.companyId);

    const handleAddLog = () => {
        if (!logOutcome || !company) return alert("Error: Company link not found.");
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center p-4">
            <Card className="w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col md:flex-row gap-6 bg-white p-0 shadow-2xl">
                <div className="flex-1 p-6 overflow-y-auto space-y-4">
                    ...
                    <Textarea label="Deal Notes" rows="6" value={opp.notes || ''} readOnly className="bg-gray-50" />
                </div>

                <div className="flex-1 border-l border-gray-200 bg-slate-50 p-6 flex flex-col overflow-hidden">
                    <div className="flex gap-2 mb-4">
                        <button onClick={() => setActiveTab('ACTIVITY')} className={`px-4 py-2 rounded-lg text-sm font-bold ${activeTab === 'ACTIVITY' ? 'bg-orange-100 text-orange-700 shadow-sm' : 'text-gray-400 hover:bg-white'}`}>Activity Log (Synced)</button>
                        <button onClick={() => setActiveTab('DATA')} className={`px-4 py-2 rounded-lg text-sm font-bold ${activeTab === 'DATA' ? 'bg-blue-100 text-blue-700 shadow-sm' : 'text-gray-400 hover:bg-white'}`}>Linked Data</button>
                    </div>
                        ...
                        {activeTab === 'ACTIVITY' ? (
                            <div className="space-y-4">
                                <div className="bg-white p-4 rounded-xl border shadow-sm space-y-3">
                                    <div className="flex gap-2">
                                        <Input type="date" value={logDate} onChange={e => setNewLogDate(e.target.value)} className="text-xs" />
                                    ...
                                    <div className="flex gap-2">
                                        <input type="text" value={logOutcome} onChange={e => setLogOutcome(e.target.value)} placeholder="Log outcome..." className="flex-1 px-3 py-2 border rounded-md text-sm" />
                            ...
                            <div className="space-y-4">
                                <div className="bg-white p-4 rounded-xl border">
                                    <h5 className="font-bold text-xs text-gray-400 uppercase mb-3 flex items-center gap-2"><FileText size={14}/> Associated Quotes</h5>
                                    {relevantQuotes.length === 0 ? <p className="text-xs italic text-gray-300">No linked quotes</p> : relevantQuotes.map(q => (
                                        <div key={q.id} className="flex justify-between items-center text-xs py-2 border-b last:border-0 font-bold">
                                            <span>{q.id}</span>
...
const FunnelPage = ({ opportunities = [], user, companies = [], contacts = [], quotes = [] }) => { 
    ...
    // ✅ THE SYNC FIX: Saves interaction to the COMPANY collection, not the lead
    ...
    return (
        <div className="w-full">
            ...
            {showDetailModal && selectedOpp && <OpportunityDetailModal opp={selectedOpp} companies={companies} quotes={quotes} contacts={contacts} onSaveInteraction={handleSaveInteraction} onClose={() => setShowDetailModal(false)} />}
