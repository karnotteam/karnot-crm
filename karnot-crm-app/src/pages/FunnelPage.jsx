...
const OpportunityDetailModal = ({ opp, onClose, onSaveInteraction, onDeleteInteraction, onUpdateProb, onUpdateNotes, quotes = [], companies = [], onOpenQuote }) => {
    ...
    // ROBUST LINKING LOGIC - FIXING THE VISIBILITY ISSUE
    const relevantQuotes = useMemo(() => {
        ...
        return (quotes || []).filter(q => {
            ...
            // Check by ID (opportunityId or leadId) OR fuzzy name match
            return (
                q.opportunityId === opp.id || 
                q.leadId === opp.id || 
                (targetName !== "" && (quoteName.includes(targetName) || targetName.includes(quoteName)))
            );
        });
    }, [quotes, opp]);
            ...
            <Card className="w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col md:flex-row shadow-2xl bg-white p-0">
                ...
                <div className="flex-1 p-6 overflow-y-auto space-y-4 border-r">
                    ...
                    <div className="bg-orange-50 p-4 rounded-xl border border-orange-100 font-bold text-gray-800">
                        <p className="text-[10px] text-orange-400 uppercase mb-1">Project Name</p>
                        {opp.project}
                    </div>
                ...
                {/* RIGHT COLUMN: TABS & LOGGING */}
                <div className="flex-1 bg-slate-50 flex flex-col overflow-hidden">
                    <div className="flex border-b bg-white">
                        <button 
                            onClick={() => setActiveTab('ACTIVITY')} 
                            className={`flex-1 py-4 text-[10px] font-black uppercase tracking-[0.2em] ${activeTab === 'ACTIVITY' ? 'text-orange-600 border-b-4 border-orange-600' : 'text-gray-400'}`}
                        >
                            Activity Log
                        </button>
                        <button 
                            onClick={() => setActiveTab('DATA')} 
                            className={`flex-1 py-4 text-[10px] font-black uppercase tracking-[0.2em] ${activeTab === 'DATA' ? 'text-blue-600 border-b-4 border-blue-600' : 'text-gray-400'}`}
                        >
                        ...
                        {activeTab === 'ACTIVITY' ? (
                            <div className="space-y-4">
                                ...
                                <div className="bg-white p-4 rounded-xl border shadow-sm space-y-3">
                                    <div className="flex gap-2">
                                        ...
                                        <select 
                                            ...
                                            className="text-xs border rounded-xl p-1 flex-1 font-black uppercase bg-gray-50 outline-none"
                                        >
                                    ...
                                    <div className="flex gap-2">
                                        <input 
                                            ...
                                            className="flex-1 text-sm p-2 border rounded-xl" 
                                        />
                                        <Button onClick={handleAddLog} variant="primary">
                                            <PlusCircle size={20}/>
                                        </Button>
                                    </div>
                                </div>
                                ...
                                {interactions.map(log => (
                                    <div key={log.id} className="bg-white p-4 rounded-xl border shadow-sm flex justify-between items-start group">
                                        <div className="flex-1">
                                            <div className="flex justify-between items-center mb-1">
                                                <span className={`text-[9px] font-black px-2 py-0.5 rounded-full text-white uppercase tracking-widest ${log.type === 'Visit' ? 'bg-green-500' : 'bg-blue-500'}`}>
                                                    {log.type}
                                                </span>
                                        ...
                                        <button 
                                            onClick={() => onDeleteInteraction(company.id, log.id)} 
                            ...
                            <div className="space-y-4">
                                {relevantQuotes.length === 0 ? (
                                    <div className="text-center py-10 text-gray-300">
                                        ...
                                        <p className="text-xs font-bold uppercase tracking-widest">No matching quotes found</p>
                                    </div>
                                ) : (
                                    relevantQuotes.map(q => (
                                        <div 
                                            ...
                                            className="p-4 bg-white border border-slate-200 rounded-2xl cursor-pointer hover:border-orange-500 transition-all flex justify-between items-center group"
                                        >
                                            <div>
                                                <p className="text-gray-800 font-black text-xs uppercase">{q.id}</p>
                                                <p className="text-[9px] text-gray-400 font-bold">VIEW PROPOSAL →</p>
                                            </div>
                ...
                {STAGE_ORDER.map(stage => {
                    ...
                    return (
                        <div key={stage} className="flex-shrink-0 w-80 bg-gray-200/50 p-4 rounded-3xl border border-gray-200/50">
                            <h3 className="font-black text-slate-500 text-[10px] uppercase tracking-[0.3em] mb-4">
                                {stage} ({stageOpps.length})
                            </h3>
                            ...
                            <div className="space-y-4">
                                {stageOpps.map(opp => {
                                    ...
                                    return (
                                        <Card key={opp.id} className="p-5 rounded-2xl shadow-sm bg-white hover:border-orange-400 group transition-all">
                                            ...
                                            <Button 
                                                ...
                                                variant="secondary" 
                                                className="w-full !py-2 text-[9px] font-black uppercase tracking-widest mb-3"
                                            >
                                                Interactions & Quotes
                                            </Button>
                                            ...
                                            <div className="flex gap-2">
                                                {currIdx > 0 && (
                                                    <button 
                                                        onClick={() => handleUpdateStage(opp.id, STAGE_ORDER[currIdx - 1])} 
                                                        className="flex-1 py-2 text-[9px] font-black text-gray-400 bg-gray-50 rounded-lg uppercase border hover:bg-white transition-colors"
                                                    >
                                                        Back
                                                    </button>
                                                )}
                                                {currIdx < STAGE_ORDER.length - 1 && (
                                                    <button 
                                                        onClick={() => handleUpdateStage(opp.id, STAGE_ORDER[currIdx + 1])} 
                                                        className="flex-1 py-2 text-[9px] font-black text-blue-600 bg-blue-50 rounded-lg uppercase border border-blue-100 hover:bg-blue-100 transition-colors"
                                                    >
                                                        Forward
                                                    </button>
                                                )}
