...
import { MapPin, Download, Search, Target, Zap, Building } from 'lucide-react';
...
const TerritoryLeadGenerator = ({ territories = [] }) => {
    ...
    const CATEGORY_CONFIG = {
        ...
        supermarkets: {
            ...
            keywords: ['SM', 'Robinsons', 'Puregold', 'supermarket'],
            score: 4,
            icon: '🛒',
        ...
        icePlants: {
            label: 'Ice Plants',
            placeTypes: ['food'],
            keywords: ['ice plant', 'ice factory'],
            score: 3,
            icon: '🧊',
    ...
    // CALL NETLIFY FUNCTION INSTEAD OF GOOGLE API DIRECTLY
    const searchGooglePlaces = async (center, category, radius) => {
        const config = CATEGORY_CONFIG[category];
        
        try {
            console.log(`🔍 Searching for ${category} in ${center.name}...`);
            
            // Call our Netlify Function (NOT Google API directly)
            const response = await fetch('/.netlify/functions/places-search', {
                ...
                body: JSON.stringify({
                    ...
                    keyword: config.keywords.join(' ') // Combine keywords
                })
            });
            ...
            if (data.status === 'OK' && data.results && data.results.length > 0) {
                console.log(`✅ Found ${data.results.length} ${category}`);
                ...
                console.log(`❌ No ${category} found in ${center.name}`);
                return [];
            } else {
    ...
    const generateLeads = async () => {
        ...
        try {
            ...
            if (uniqueLeads.length === 0) {
                alert('No leads found. Try increasing the search radius or selecting different categories.');
            }
        ...
        <div className="space-y-6">
            ...
            <Card className="bg-green-50 border-2 border-green-200">
                <div className="flex items-start gap-3">
                    ...
                    <div>
                        <h4 className="font-black uppercase text-xs text-green-800 mb-2">✅ Production Ready</h4>
                        <p className="text-xs text-green-700 leading-relaxed">
                            This system uses a secure Netlify Function to call Google Places API. 
                            Your API key is safely stored in environment variables and never exposed to the browser.
                        </p>
                    </div>
                </div>
            </Card>
