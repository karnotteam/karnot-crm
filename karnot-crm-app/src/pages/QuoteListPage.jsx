...
import { QUOTE_STATUSES, Card, Button } from '../data/constants.jsx';

// This is your SavedQuotesList component, moved from App.jsx
const QuotesListPage = ({ quotes, onUpdateQuoteStatus, onDeleteQuote, onEditQuote }) => {
    ...
    const getQuoteDate = (quote) => {
        ...
        // Handle ISO strings (from old localStorage data)
                        ...
                        {quotes.sort((a, b) => getQuoteDate(b) - getQuoteDate(a)).map(quote => (
                            <React.Fragment key={quote.id}>
                                <tr className="border-b hover:bg-gray-50 cursor-pointer" onClick={() => setExpandedQuoteId(expandedQuoteId === quote.id ? null : quote.id)}>
                                    ...
                                    {/* FIX: Added '?' to prevent crash if customer is missing */}
                                    <td className="p-3">{quote.customer?.name}</td>
                                    <td className="p-3"><span className={`px-2 py-1 text-xs font-semibold text-white rounded-full ${QUOTE_STATUSES[quote.status]?.color || 'bg-gray-400'}`}>{QUOTE_STATUSES[quote.status]?.text || quote.status}</span></td>
                                ...
                                {expandedQuoteId === quote.id && (
                                    <tr className="bg-gray-100"><td colSpan="7" className="p-4">
                                        <div className="flex justify-between items-center">
                                            {/* FIX: Use helper function to display date */}
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                ...
                                                {/* FIX: Added confirmation before deleting */}
