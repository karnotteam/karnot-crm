...
const HeatPumpCalculator = ({ leadId }) => {  
  ...
  // --- NEW: AUTO-UPDATE RATES WHEN CURRENCY CHANGES ---
  useEffect(() => {
  ...
  const generateReport = () => {
      ...
      const reportHTML = `
        <!DOCTYPE html><html><head><title>Karnot Savings Report</title><style>body{font-family:sans-serif;padding:40px;color:#1d1d1f}.header{text-align:center;border-bottom:2px solid #F56600;margin-bottom:30px}h1{color:#F56600}.summary{display:grid;grid-template-columns:1fr 1fr 1fr;gap:20px;text-align:center;margin:30px 0}.metric-val{font-size:24px;font-weight:bold;color:#F56600}.details{width:100%;border-collapse:collapse}.details td{padding:12px;border-bottom:1px solid #eee}</style></head>
        <body><div class="header"><h1>Karnot Savings Report</h1><p>Internal Estimate for ${q.system.n}</p></div>
        <h2>System Recommendation: ${q.system.n}</h2>
        <div class="summary"><div><div class="metric-val">${q.financials.symbol}${fmt(q.financials.totalSavings)}</div><div>Annual Savings</div></div><div><div class="metric-val">${q.financials.paybackYears} Yrs</div><div>Payback Period</div></div><div><div class="metric-val">Active</div><div>Emission Reduction</div></div></div>
        <table class="details"><tr><td>Annual Cost (Old)</td><td align="right">${q.financials.symbol}${fmt(q.financials.annualCostOld)}</td></tr><tr><td>Annual Cost (New)</td><td align="right">${q.financials.symbol}${fmt(q.financials.karnotAnnualCost)}</td></tr>${coolSavingsRow}<tr><td><b>Total Savings</b></td><td align="right"><b>${q.financials.symbol}${fmt(q.financials.totalSavings)}</b></td></tr></table>
        <p>Estimate based on system cost of ${q.financials.symbol}${fmt(q.financials.capex.total)}.</p></body></html>`;
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <Section title="1. Your Demand">
                <div className="space-y-4">
                    ...
                    {isOccupantFieldVisible && <Input label="Number of Occupants" type="number" value={inputs.occupants} onChange={handleChange('occupants', true)} />}
            ...
            <Section title="2. Your Costs">
                <div className="space-y-4">
                    ...
                    {inputs.heatingType === 'propane' ? <div className="flex gap-2 items-center"><Input type="number" value={inputs.fuelPrice} onChange={handleChange('fuelPrice', true)} /><span>for a</span><Input type="number" value={inputs.tankSize} onChange={handleChange('tankSize', true)} /><span>kg tank</span></div> : <Input type="number" value={inputs.fuelPrice} onChange={handleChange('fuelPrice', true)} />}
            ...
            <Section title="3. Conditions">
                <div className="space-y-4">
                    <Input label="Ambient Air Temp (°C)" type="number" value={inputs.ambientTemp} onChange={handleChange('ambientTemp', true)} />
                    ...
                    {isSunHoursVisible && <Input label="Avg Sun Hours" type="number" value={inputs.sunHours} onChange={handleChange('sunHours', true)} />}
                    <label>HP Type</label>
                    <select className="w-full border p-2 rounded" value={inputs.heatPumpType} onChange={handleChange('heatPumpType')}><option value="all">All Models</option><option value="R290">R290 Only</option><option value="R744">CO2 Only</option><option value="R32">R32 Only</option></select>
                </div>
            </Section>
        </div>
        ...
        {result && !result.error && (
            <div className="mt-8 bg-slate-50 p-6 rounded-xl border border-slate-200">
                <div className="flex justify-between items-end mb-6">
                    <div><h3 className="text-xl font-bold text-orange-600">{result.system.n}</h3><p className="text-sm text-gray-500">Load: {result.metrics.requiredThermalPowerKW} kW</p></div>
                ...
                {result.financials.coolSavings > 0 && <div className="bg-blue-50 border-l-4 border-blue-500 p-4 mb-4"><h4 className="text-blue-700 font-bold text-sm">Free Cooling Bonus!</h4><p className="text-sm text-blue-900">Plus ${result.financials.symbol}${fmt(result.financials.coolSavings)} savings!</p></div>}
                <table className="w-full text-sm bg-white border rounded-lg">
                    <tr className="border-b"><td className="p-3">Annual Cost (Old)</td><td className="p-3 text-right">{result.financials.symbol}${fmt(result.financials.annualCostOld)}</td></tr>
                    <tr className="border-b"><td className="p-3">Annual Cost (New)</td><td className="p-3 text-right">{result.financials.symbol}${fmt(result.financials.karnotAnnualCost)}</td></tr>
                    <tr className="font-bold bg-gray-50"><td className="p-3">Total Savings</td><td className="p-3 text-right text-green-600">{result.financials.symbol}${fmt(result.financials.totalSavings)}</td></tr>
                    <tr className="font-bold"><td className="p-3">Payback</td><td className="p-3 text-right text-orange-600">{result.financials.paybackYears} Yrs</td></tr>
                </table>
                <div className="flex justify-end gap-3 mt-6"><Button onClick={generateReport} variant="secondary"><Printer size={18}/> Report</Button><Button onClick={handleSave} variant="success" disabled={isSaving}><Save size={18}/> {isSaving ? 'Saving...' : 'Save'}</Button></div>
            </div>
        )}
