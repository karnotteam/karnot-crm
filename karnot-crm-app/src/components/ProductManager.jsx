...
import { Plus, Search, Edit, Trash2, X, Save, Package, Upload, Download, Thermometer, Box, RefreshCw, Layers } from 'lucide-react';
...
const ProductManager = ({ user }) => {
    ...
    const defaultFormData = {
        ...
        max_temp_c: 65, Refrigerant: '', Power_Supply: '', Rated_Power_Input: '',
        ...
        Papa.parse(file, {
            ...
            complete: async (results) => {
                ...
                results.data.forEach((row, index) => {
                    const sysId = row['System ID']?.trim() || row['id']?.trim() || `PROD_${Date.now()}_${index}`;
                ...
                await batch.commit();
                alert(`SUCCESS: Imported ${results.data.length} items.`);
            }
        });
    };
    ...
    return (
        <div className="w-full space-y-6 pb-20">
            ...
            {isEditing && (
                <Card className="p-8 border-t-8 border-orange-500 shadow-2xl bg-white">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                        ...
                        <div className="flex items-center pt-6"><Checkbox label="Cooling Capable" checked={formData.isReversible} onChange={e => setFormData({...formData, isReversible: e.target.checked})} /></div>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                        <Textarea label="Technical Description" rows="4" value={formData.Specs} onChange={e => setFormData({...formData, Specs: e.target.value})} />
            ...
            <div className="relative">
                <input type="text" placeholder="Search 300+ models..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-14 pr-4 py-5 bg-white border-2 border-slate-100 rounded-3xl shadow-sm focus:border-orange-500 outline-none font-bold text-slate-700" />
                        ...
                        {filtered.map(p => (
                            <tr key={p.id} className="hover:bg-orange-50/50 transition-all group">
                                <td className="px-10 py-8">
                                    ...
                                    <div className="flex items-center gap-3 text-[10px] font-black text-orange-600 uppercase tracking-widest">{p.category} • {p.id}</div>
                                </td>
                                <td className="px-10 py-8">
                                    <div className="flex gap-2">
