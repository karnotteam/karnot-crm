    ...
    <header className="bg-white shadow-md sticky top-0 z-50 border-b-2 border-orange-500">
        <div className="container mx-auto px-4 py-4 flex flex-col lg:flex-row justify-between items-center gap-4">
            ...
            <nav className="flex flex-wrap gap-2 justify-center lg:justify-end">
                ...
                <Button onClick={() => setActiveView('commissioning')} variant={activeView === 'commissioning' ? 'primary' : 'secondary'} className="font-bold uppercase text-[10px] tracking-widest"><ClipboardCheck className="mr-1" size={14} /> Reports</Button>
                <Button onClick={() => setActiveView('calculatorsHub')} variant={['calculatorsHub', 'heatPumpCalc'].includes(activeView) ? 'primary' : 'secondary'} className="font-bold uppercase text-[10px] tracking-widest"><Calculator className="mr-1" size={14} /> Tools</Button>
                
                <div className="h-8 w-px bg-gray-200 mx-2 hidden lg:block"></div>

                <Button onClick={onNewQuote} variant="primary" className="font-black uppercase text-[10px] tracking-widest shadow-lg shadow-orange-100 bg-orange-600"><Plus className="mr-1" size={14} /> New Quote</Button>
...
export default function App() {
    ...
    if (loadingAuth) return <div className="text-center p-10 font-black uppercase tracking-widest">Authenticating...</div>;
    if (!user) return <LoginPage onLogin={handleLogin} />;
    if (loadingData) return <div className="text-center p-10 font-black uppercase tracking-widest">Loading Karnot Systems...</div>;
    ...
    return (
        <div className="bg-gray-100 min-h-screen font-sans text-gray-900">
            ...
            <main className="container mx-auto p-4 md:p-8">
                ...
                {activeView === 'list' && <QuotesListPage quotes={quotes} onDeleteQuote={handleDeleteQuote} onEditQuote={handleEditQuote} />}
                ...
                {activeView === 'commissioning' && <CommissioningPage user={user} companies={companies} contacts={contacts} onBack={handleBackToFunnel} />}
