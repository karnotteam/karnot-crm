...
const BIRBookPrep = ({ quotes = [], ledgerEntries = [] }) => {
    ...
    // We must map the quotes to calculate their specific PHP values using their saved Forex rates
    const salesDataPHP = invoicedQuotes.map(q => {
        const rate = q.costing?.forexRate || 58.5; // Fallback to 58.5 if rate is missing
        const grossPHP = (q.finalSalesPrice || 0) * rate;
        const netPHP = grossPHP / 1.12;
        const vatPHP = grossPHP - netPHP;
        return { ...q, grossPHP, netPHP, vatPHP };
    });

    const totalGrossSalesPHP = salesDataPHP.reduce((sum, q) => sum + q.grossPHP, 0);
    const totalOutputVatPHP = salesDataPHP.reduce((sum, q) => sum + q.vatPHP, 0);

    // --- 2. DISBURSEMENT JOURNAL MATH (Input VAT) ---
    const totalInputVatPHP = ledgerEntries.reduce((sum, e) => {
    ...
    const netVatPayablePHP = totalOutputVatPHP - totalInputVatPHP;
        ...
        <div className="space-y-10 pb-20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="p-4 bg-white border border-gray-200 rounded shadow-sm hover:shadow-md transition-shadow">
                    ...
                    <p className="text-xl font-black text-gray-800">₱{totalOutputVatPHP.toLocaleString(undefined, {minimumFractionDigits: 2})}</p>
                    <p className="text-[9px] text-gray-400 mt-1 italic">Converted Total Output VAT</p>
                </div>
                <div className="p-4 bg-white border border-gray-200 rounded shadow-sm hover:shadow-md transition-shadow">
                    <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Box 20B (Input VAT)</p>
                    <p className="text-xl font-black text-blue-600">₱{totalInputVatPHP.toLocaleString(undefined, {minimumFractionDigits: 2})}</p>
                    <p className="text-[9px] text-gray-400 mt-1 italic">VAT from Domestic Purchases</p>
                </div>
                <div className={`p-4 rounded shadow-lg text-white md:col-span-2 flex flex-col justify-center ${netVatPayablePHP >= 0 ? 'bg-slate-900' : 'bg-green-700'}`}>
                    <p className="text-[10px] text-orange-400 font-black uppercase tracking-[0.2em]">Box 26 (Net VAT Payable / Excess)</p>
                    <div className="flex items-baseline gap-2">
                        <p className="text-2xl font-black">₱{netVatPayablePHP.toLocaleString(undefined, {minimumFractionDigits: 2})}</p>
                        <span className="text-[10px] opacity-70 font-bold uppercase">{netVatPayablePHP >= 0 ? 'To Pay' : 'Excess Input'}</span>
                    </div>
                </div>
            </div>
            ...
            <Card className="border-0 shadow-xl overflow-hidden">
                <div className="p-4 bg-gray-50 border-b">
                    <h2 className="text-xs font-black text-gray-500 uppercase tracking-[0.2em]">Sales Journal (6-Column Book Data - PHP Converted)</h2>
                </div>
                        ...
                        <tbody className="divide-y divide-gray-100 bg-white">
                            {salesDataPHP.map(q => (
                                <tr key={q.id} className="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td className="p-4 text-right font-mono">₱{q.netPHP.toLocaleString(undefined, {minimumFractionDigits: 2})}</td>
                                    <td className="p-4 text-right font-mono text-gray-500">₱{q.vatPHP.toLocaleString(undefined, {minimumFractionDigits: 2})}</td>
                                    <td className="p-4 text-right font-black bg-gray-50/50">₱{q.grossPHP.toLocaleString(undefined, {minimumFractionDigits: 2})}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
